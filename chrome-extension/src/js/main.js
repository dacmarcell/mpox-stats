...
function applyDarkTheme(body, table) {
  body.classList.replace('light', 'dark')
  table.classList.replace('table-striped', 'table-dark')

  const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-sun" viewBox="0 0 16 16">
  ...
  document.getElementById('toggle-icon-container').innerHTML = svg
}

function applyLightTheme(body, table) {
  body.classList.replace('dark', 'light')
  table.classList.replace('table-dark', 'table-striped')

  const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-moon" viewBox="0 0 16 16">
